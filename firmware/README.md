...
Layout follows the current 400×300 dashboard: header (time | indoor temp/RH |
pet | outdoor weather), a lower two-panel carousel rotating Claude,
DeepSeek, and Codex usage, plus a BOOT-key large pet animation page. Icons in
`ui_app/icons.c` are generated by `scripts/gen_icons.py` from
`docs/assets/*.svg` (Claude Code robot, Codex, DeepSeek whale) plus drawn
weather glyphs.
...
2. **Live data**: switch to no query string, run Claude Code on the bridge
   host for a minute, watch today's Claude or Codex tokens count up on the next poll.
